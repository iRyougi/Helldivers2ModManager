...
internal sealed class LocalizationService : INotifyPropertyChanged
{
	...
	private async Task CreateDefaultLanguageFilesAsync()
	{
		...
		var enTranslations = new Dictionary<string, string>
		{
			...
			["Settings.GameDirDesc"] = "This is where your Helldivers 2 game is installed.",
			["Settings.StorageDir"] = "Storage Directory",
			["Settings.StorageDirDesc"] = "This is where all manager and mod related files are stored.",
			["Settings.TempDir"] = "Temporary Directory",
			["Settings.TempDirDesc"] = "This is where temporary files are stored during extraction.",
		...
		var zhCnTranslations = new Dictionary<string, string>
		{
			...
			["Settings.GameDirDesc"] = "这是您的Helldivers 2游戏安装位置。",
			["Settings.StorageDir"] = "存储目录",
			["Settings.StorageDirDesc"] = "这是存储所有管理器和模组相关文件的位置。",
			["Settings.TempDir"] = "临时目录",
			["Settings.TempDirDesc"] = "这是在提取过程中存储临时文件的位置。",
